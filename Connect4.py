...
"""
Runs the game, pitting the user against an AI
"""
def main():
    ...
    depth_input = "a"
    while not depth_input in "12345" or len(depth_input) == 0:
        depth_input = input("AI difficulty (1 - 5): ")
    max_depth = int(ord(depth_input[0])-ord("0"))
    ...
    while not game_over:
        ...
        print("Player ", player, "'s turn (", piece, ")", sep = "")
        ...
        # Player's turn
            ...
            while not move_made:
                ...
                col = -1
                while row == -1:
                    player_move = str.lower(input("Move (e.g. B): "))
                    if (player_move in "abcdefg") and not len(player_move) == 0:
                        col = ord(player_move[0])-ord("a")
                        row = lowest_row(board, col)
                try:
                    make_move(board, player_piece, row, col)
                    move_made = True
